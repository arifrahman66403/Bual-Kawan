...
<div class="slider">
    @if ($sliders->count() > 0)
        @foreach ($sliders as $slider)
            <div class="slide {{ $loop->first ? 'active' : '' }}">
                {{-- Asumsi kolom untuk gambar adalah 'image_path' --}}
                <img src="{{ Storage::url($slider->image) }}" alt="{{ $slider->title }}">
                <div class="slide-info">
                    {{-- Asumsi kolom untuk Judul dan Keterangan adalah 'title' dan 'caption' --}}
                    <h2>{{ $slider->title }}</h2>
    ...
    @else
        ...
        <div class="slide active">
            <img src="https://picsum.photos/id/1001/1600/900" alt="Default Slide">
            <div class="slide-info">
    ...
    {{-- Tombol Navigasi Slider (Tetap Statis untuk kontrol JS) --}}
...
<x-layout title="Beranda">
    ...
    {{-- =========================================== --}}
    {{-- 3. SEKSI BERITA TERKINI (STATIS) --}}
    {{-- Note: Data Berita masih placeholder. Jika ada model Berita, logikanya akan mirip dengan Galeri. --}}
    {{-- =========================================== --}}
    <section class="berita">
        <h2 data-aos="fade-up">Berita Terkini</h2>
        <div class="berita-wrapper">
            <div class="berita-container" id="beritaSlider">
                {{-- Data berita placeholder --}}
    ...
    {{-- =========================================== --}}
    {{-- 4. GALERI KEGIATAN --}}
    {{-- Menggunakan data dinamis dari $galleries --}}
    {{-- =========================================== --}}
        ...
        <div class="masonry">
            @if ($galleries->count() > 0)
                @foreach ($galleries as $gallery)
                    {{-- Asumsi $gallery->image_path berisi path ke gambar --}}
                ...
                @endforeach
            @else
                <div class="text-center w-100">Belum ada foto galeri yang diunggah.</div>
            @endif
        </div>
    </section>

    {{-- =========================================== --}}
    {{-- 5. JAVASCRIPT (TIDAK ADA PERUBAHAN FUNGSIONAL) --}}
    {{-- Hanya memastikan JS SLIDER UTAMA tetap bekerja dengan class dinamis --}}
    {{-- =========================================== --}}
    ...
    <script>
        ...
        // Note: Script ini akan bekerja karena kita menggunakan class '.slide' dan '.active' yang sama.
        ...
        function checkScrollPosition() {
            ...
            // Selalu tampilkan tombol (jika ada)
            if(scrollToBtn) scrollToBtn.classList.add('show');
            ...
            if (window.innerWidth <= 992 && scrollTop < 100) {
                if(scrollToBtn) scrollToBtn.classList.remove('show');
            }
        }
